...
function ProviderListingNewPageInner() {
  ...
  // Pre-fill from signup query params
  useEffect(() => {
    setForm((prev) => ({
      ...prev,
      contactName: params.get('name') || '',
  ...
  async function submit() {
    ...
    try {
      const res = await fetch('/api/providers/submit', {
        ...
        body: JSON.stringify({ type: 'listing', ...form }),
      });
  ...
  return (
    <main className="min-h-screen bg-[#F5EFE0] flex flex-col items-center px-4 py-16 pt-28">
      ...
      <div className="text-center mb-8">
        <Link href="/providers/signup" className="inline-flex items-center gap-1.5 text-[13px] text-[#1A7A8A] font-medium mb-5 hover:underline">
      ...
      <div className="bg-white rounded-[24px] shadow-[0_4px_32px_rgba(10,74,90,0.09)] w-full max-w-[560px] p-8 md:p-10">
        ...
        {step === 3 && <StepReview form={form} errors={errors} />}
...
function StepDetails({ form, set, errors }: { form: ListingForm; set: (f: keyof ListingForm, v: string) => void; errors: Errors }) {
  return (
...
function StepPricing({ form, set, errors }: { form: ListingForm; set: (f: keyof ListingForm, v: string) => void; errors: Errors }) {
  return (
    <div className="flex flex-col gap-5">
      <div>
...
function StepLocation({ form, set, errors }: { form: ListingForm; set: (f: keyof ListingForm, v: string) => void; errors: Errors }) {
  return (
    <div className="flex flex-col gap-5">
      <div>
      ...
      <div className="border-t border-[#E8DFC8] pt-4 flex flex-col gap-1.5">
        <p className="text-[13px] font-medium text-[#0D5C6E]">Your contact info</p>
        <p className="text-[13px] text-[#7A9AAA] mb-3">Pre-filled from your account — only visible to the Little Sound team.</p>
        <div className="bg-[#F5EFE0] border border-[#E8DFC8] rounded-[12px] px-5 py-4 flex flex-col gap-1">
          <p className="text-[14px] text-[#1C3A4A] font-medium">{form.contactName || '—'}</p>
          <p className="text-[13px] text-[#3A5A6A]">{form.contactEmail || '—'}</p>
        </div>
      </div>
    </div>
  );
...
function StepReview({ form, errors }: { form: ListingForm; errors: Errors }) {
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="bg-[#0D5C6E]/5 border border-[#0D5C6E]/15 rounded-[14px] px-5 py-4">
        <p className="text-[13px] text-[#0D5C6E] leading-relaxed font-medium">
          After submission: our team reviews your listing within 24 hours, then it goes live in the Little Sound directory. You&apos;ll hear from us at {form.contactEmail}.
        </p>
      </div>
    </div>
  );
